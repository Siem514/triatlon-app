              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '12px', marginBottom: '12px' }}>
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '0.75rem', fontWeight: '800', color: '#1e40af' }}>TOTAAL GEPLAND VOLUME</span>
                    <span style={{ fontSize: '0.7rem', background: '#2563eb', color: '#ffffff', padding: '2px 6px', borderRadius: '4px', fontWeight: '700' }}>Bekijk Grafieken →</span>
                  </div>
                  <div style={{ fontSize: '1.6rem', fontWeight: '800', color: '#1e3a8a', marginTop: '4px' }}>{weeklyVolume.total} Uur</div>
                  <div style={{ fontSize: '0.72rem', color: '#2563eb', marginTop: '6px', fontWeight: '600' }}>
                    🏃 Lopen: {weeklyVolume.run}u | 🚴 Fietsen: {weeklyVolume.bike}u | 🏊 Zwemmen: {weeklyVolume.swim}u
                  </div>
                </button>
                
                <button 
                  onClick={() => setShowFeedbackModal(true)}
                  style={{ background: unreadFeedbackCount > 0 ? '#ecfdf5' : '#f8fafc', padding: '14px', borderRadius: '10px', border: unreadFeedbackCount > 0 ? '2px solid #10b981' : '1px solid #e2e8f0', textAlign: 'left', cursor: 'pointer' }}
                >
                  <span style={{ fontSize: '0.75rem', fontWeight: '700', color: unreadFeedbackCount > 0 ? '#065f46' : '#64748b' }}>📩 RPE FEEDBACK BERICHTEN</span>
                  <div style={{ fontSize: '1.3rem', fontWeight: '800', color: unreadFeedbackCount > 0 ? '#047857' : '#0f172a', marginTop: '4px' }}>
                    {unreadFeedbackCount > 0 ? `💬 ${unreadFeedbackCount} Nieuwe Inzending(en)` : 'Bekijk Alle Feedback'}
                  </div>
                  <div style={{ fontSize: '0.72rem', color: unreadFeedbackCount > 0 ? '#10b981' : '#64748b', marginTop: '6px', fontWeight: '700' }}>
                    Klik om te openen →
                  </div>
                </button>
              </div>
          ...
          <div>
            <div style={{ background: '#ffffff', borderRadius: '12px', padding: '20px', border: '1px solid #e2e8f0', marginBottom: '16px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', borderBottom: '2px solid #f1f5f9', paddingBottom: '10px' }}>
                <div>
                  <h3 style={{ fontSize: '1.2rem', fontWeight: '800', color: '#0f172a', margin: 0 }}>📊 Trainingsvolume & Grafieken per Sport</h3>
                  <p style={{ margin: '2px 0 0 0', fontSize: '0.82rem', color: '#64748b' }}>Week van {weekDates['Maandag']} t/m {weekDates['Zondag']}</p>
                </div>
                <div style={{ background: '#eff6ff', color: '#1e40af', padding: '6px 12px', borderRadius: '8px', fontWeight: '800', fontSize: '1rem' }}>
                  Totaal: {weeklyVolume.total} Uur
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gap: '16px' }}>
                <div style={{ background: '#f8fafc', padding: '14px', borderRadius: '10px', border: '1px solid #cbd5e1' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                    ...
                    <strong style={{ fontSize: '1rem', color: '#0f172a' }}>{weeklyVolume.run} Uur ({weeklyVolume.total > 0 ? Math.round((weeklyVolume.run / weeklyVolume.total) * 100) : 0}%)</strong>
                  </div>
                  <div style={{ width: '100%', height: '14px', background: '#e2e8f0', borderRadius: '999px', overflow: 'hidden' }}>
                    <div style={{ width: `${weeklyVolume.total > 0 ? (weeklyVolume.run / weeklyVolume.total) * 100 : 0}%`, height: '100%', background: '#2563eb', borderRadius: '999px', transition: 'width 0.4s ease' }}></div>
                  </div>
                </div>
                ...
                <div style={{ background: '#f8fafc', padding: '14px', borderRadius: '10px', border: '1px solid #cbd5e1' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                    ...
                    <strong style={{ fontSize: '1rem', color: '#0f172a' }}>{weeklyVolume.bike} Uur ({weeklyVolume.total > 0 ? Math.round((weeklyVolume.bike / weeklyVolume.total) * 100) : 0}%)</strong>
                  </div>
                  <div style={{ width: '100%', height: '14px', background: '#e2e8f0', borderRadius: '999px', overflow: 'hidden' }}>
                    <div style={{ width: `${weeklyVolume.total > 0 ? (weeklyVolume.bike / weeklyVolume.total) * 100 : 0}%`, height: '100%', background: '#16a34a', borderRadius: '999px', transition: 'width 0.4s ease' }}></div>
                  </div>
                </div>
                ...
                <div style={{ background: '#f8fafc', padding: '14px', borderRadius: '10px', border: '1px solid #cbd5e1' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                    ...
                    <strong style={{ fontSize: '1rem', color: '#0f172a' }}>{weeklyVolume.swim} Uur ({weeklyVolume.total > 0 ? Math.round((weeklyVolume.swim / weeklyVolume.total) * 100) : 0}%)</strong>
                  </div>
                  <div style={{ width: '100%', height: '14px', background: '#e2e8f0', borderRadius: '999px', overflow: 'hidden' }}>
            ...
            <div style={{ background: '#ecfdf5', border: '1px solid #a7f3d0', borderRadius: '10px', padding: '16px', marginBottom: '16px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '10px' }}>
                <div style={{ background: '#ffffff', padding: '10px 14px', borderRadius: '8px', border: '1px solid #a7f3d0' }}>
                  <div style={{ fontSize: '0.75rem', color: '#047857', fontWeight: '700' }}>ISOTONISCHE GELS</div>
